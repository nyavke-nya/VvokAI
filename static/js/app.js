...
const SETTINGS_META = {
    general: [
        ...
        { key: "brawl_api_token", label: "Brawl Stars API Token", type: "text", placeholder: "eyJ0eXAiOiJKV1Qi...", help: "Free from developer.brawlstars.com. Log in, open My Account, Create New Key. Try putting 0.0.0.0/0 in the Allowed IP Ranges box: if the form takes it the key works from anywhere and never needs replacing. It is worth trying rather than a guarantee - the field is meant for one address, and the portal API refuses ranges outright. If it will not take it, the key is tied to the address you are on now and dies when your provider changes it, so fill in the two fields below and the bot will reissue it for you. Paste the whole key here. (Win streaks are not published by the API, so those stay as you set them.)" },
        { key: "brawl_api_email", label: "Developer Portal Email", type: "text", placeholder: "you@example.com", help: "Needed when your key is tied to a single IP address, which is the usual case. With this filled in, the bot logs in and re-issues the key by itself whenever your address changes, rather than quietly losing trophy sync. Leave empty if you managed to create a 0.0.0.0/0 key - nothing here is needed then." },
        { key: "brawl_api_password", label: "Developer Portal Password", type: "password", placeholder: "Leave empty if your key uses 0.0.0.0/0", help: "The same password you use on developer.brawlstars.com. Stored in cfg/general_config.toml on this machine, never shown back here and never written to logs - but it is a password in a plain file. If that bothers you, try the 0.0.0.0/0 key instead and leave this empty." },

...
(() => {
    ...
    const RU = {
        ...
        "Free from developer.brawlstars.com. Log in, open My Account, Create New Key. Try putting 0.0.0.0/0 in the Allowed IP Ranges box: if the form takes it the key works from anywhere and never needs replacing. It is worth trying rather than a guarantee - the field is meant for one address, and the portal API refuses ranges outright. If it will not take it, the key is tied to the address you are on now and dies when your provider changes it, so fill in the two fields below and the bot will reissue it for you. Paste the whole key here. (Win streaks are not published by the API, so those stay as you set them.)":
            "Бесплатно на developer.brawlstars.com: войти, My Account, Create New Key. Попробуй вписать 0.0.0.0/0 в поле Allowed IP Ranges: если форма примет, ключ будет работать с любого адреса и менять его не придётся. Это стоит попробовать, но не гарантия — поле рассчитано на один адрес, а API портала диапазоны отклоняет. Если не примет, ключ привязан к текущему адресу и умрёт при его смене, тогда заполни два поля ниже и бот будет перевыпускать ключ сам. Вставь ключ целиком. (Серии побед API не отдаёт, они останутся как ты их задал.)",
        "Developer Portal Email": "Почта портала разработчика",
        "Needed when your key is tied to a single IP address, which is the usual case. With this filled in, the bot logs in and re-issues the key by itself whenever your address changes, rather than quietly losing trophy sync. Leave empty if you managed to create a 0.0.0.0/0 key - nothing here is needed then.":
            "Нужна, когда ключ привязан к одному адресу, а это обычный случай. С ней бот сам зайдёт в портал и перевыпустит ключ при смене адреса, вместо того чтобы молча потерять синхронизацию. Если удалось создать ключ с 0.0.0.0/0 — оставь пустым, здесь ничего не нужно.",
        "Developer Portal Password": "Пароль портала разработчика",
        "The same password you use on developer.brawlstars.com. Stored in cfg/general_config.toml on this machine, never shown back here and never written to logs - but it is a password in a plain file. If that bothers you, try the 0.0.0.0/0 key instead and leave this empty.":
            "Тот же пароль, что на developer.brawlstars.com. Хранится в cfg/general_config.toml на этой машине, обратно сюда не показывается и в логи не пишется — но это пароль в текстовом файле. Если это смущает, попробуй ключ с 0.0.0.0/0 и оставь поле пустым.",

...
(() => {
    ...
    const RU = {
        ...
        "Free from developer.brawlstars.com. Log in, open My Account, Create New Key. IMPORTANT: in the Allowed IP Ranges box type 0.0.0.0/0 - otherwise the key only works from the address you are on right now and dies the moment your provider changes it. Paste the whole key here. (Win streaks are not published by the API, so those stay as you set them.)":
            "Бесплатно на developer.brawlstars.com: войти, My Account, Create New Key. ВАЖНО: в поле Allowed IP Ranges впиши 0.0.0.0/0 — иначе ключ работает только с того адреса, на котором ты сейчас, и умрёт, как только провайдер его сменит. Вставь ключ целиком. (Серии побед API не отдаёт, они останутся как ты их задал.)",
        "Developer Portal Email": "Почта портала разработчика",
        "Only needed if your key is tied to one IP address instead of 0.0.0.0/0. With this filled in, the bot logs in and re-issues the key by itself whenever your address changes, rather than quietly losing trophy sync. Leave empty if you used 0.0.0.0/0 - nothing here is needed then.":
            "Нужна, только если ключ привязан к одному адресу, а не к 0.0.0.0/0. С ней бот сам зайдёт в портал и перевыпустит ключ при смене адреса, вместо того чтобы молча потерять синхронизацию. Если использовал 0.0.0.0/0 — оставь пустым, здесь ничего не нужно.",
        "Developer Portal Password": "Пароль портала разработчика",
        "The same password you use on developer.brawlstars.com. Stored in cfg/general_config.toml on this machine, never shown back here and never written to logs - but it is a password in a plain file, so prefer the 0.0.0.0/0 key above if that bothers you.":
            "Тот же пароль, что на developer.brawlstars.com. Хранится в cfg/general_config.toml на этой машине, обратно сюда не показывается и в логи не пишется — но это пароль в текстовом файле, так что при сомнениях лучше ключ с 0.0.0.0/0.",

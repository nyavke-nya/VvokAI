...
Два способа с этим жить, оба в [cfg/general_config.toml](cfg/general_config.example.toml):

Сначала о том, чего делать не нужно. По интернету ходит совет вписать
`0.0.0.0/0` в поле Allowed IP Ranges и получить ключ на все адреса. **Это не
работает.** Поле принимает один адрес, а API того же портала отклоняет любой
диапазон — `0.0.0.0/0`, `/24` и `/16` все возвращают HTTP 500
`ip-validation-failure` (проверено запросами). Раньше этот совет стоял и в этом
README; он был неверен, и его убрали.

**Автоматически — то, что работает.** Заполните `brawl_api_email` и

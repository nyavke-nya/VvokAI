...
Ранние версии выполняли полторы функции из трёх: ходили, стреляли автоприцелом и никак не реагировали на летящие в бота снаряды. Здесь добавлены недостающие полторы.

**Telegram: [@nyavke](https://t.me/nyavke)**

---

## Чем отличается от ранних версий

| Функция | Раньше | VvokAI |
|---|---|---|
...
Форк раз в шесть часов отправляет обезличенные цифры: сколько матчей сыграно,
какой винрейт у бойцов и стилей, какая версия и какой провайдер вычислений.
Это нужно, чтобы понимать, что развивать, а не угадывать по тому, кто громче
написал в Discord.

**Не отправляется ничего, что указывает на человека:** ни тега игрока, ни ника,
ни API-токена, ни путей на диске. Единственное постоянное значение — случайное
число, созданное при первом запуске, чтобы одна машина не считалась за двух
человек. Отправка идёт в фоне и её сбой ничего не ломает.

Выключается одной строкой в `cfg/general_config.toml`:
...
Распакуй архив и **запусти `start_vvok.bat` двойным щелчком**. Лаунчер сам создаст venv, поставит зависимости и откроет интерфейс. Окно приложения — `desktop.py`, в браузере — `main.py`.
...
Без точки со слэшем PowerShell отвечает `Имя "start_vvok.bat" не распознано как имя командлета` — он не ищет команды в текущей папке. В `cmd.exe` работает и без неё.
...
Как это устроено и почему не тонет в шуме — подробно в **[DODGE.md](docs/DODGE.md)**.
...
`.vvok` не имеет встроенных функций, поэтому пропавший ключ контекста это не
...
`playstyles/unified_dodge.vvok` объединяет все четыре официальных плейстайла (боевое дерево, следование за союзниками, ротация к центру, архетипы) и добавляет уворот.

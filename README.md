...
Собери `VvokAI.exe` и раздавай его вместо архива:

```
tools\build_exe.bat
```

Получается **8 МБ**. Запусти его откуда угодно — хоть с рабочего стола. Проект он ставит в **`C:\VvokAI`**, туда же venv и модели; сам exe можно держать где удобно. Дальше он сам поставит Python, скачает проект и зависимости, проверит обновления и откроет окно приложения. Дальше при каждом старте проверяет обновления сам, включая обновление самого себя.

Внутри exe нет ни одной части бота: зависимости весят 3 ГБ, и такой файл распаковывался бы во временную папку при каждом запуске и ловил бы ложные срабатывания антивируса. Вся сборка — это `tools\build_exe.bat` и `launcher.py`: их можно прочитать и собрать самому, и это лучший ответ на «а вдруг это вирус», чем любые обещания.

### Из исходников

Нужен [Python 3.11.9](https://www.python.org/downloads/release/python-3119/).

Распакуй архив и **запусти `start_pyla.bat` двойным щелчком**. Лаунчер сам создаст venv, поставит зависимости и откроет интерфейс. Окно приложения — `desktop.py`, в браузере — `main.py`.

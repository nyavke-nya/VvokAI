...
**Проще, если сработает.** При создании ключа на
[developer.brawlstars.com](https://developer.brawlstars.com) попробуйте указать
`0.0.0.0/0` в поле Allowed IP Ranges. Такой ключ работает с любого адреса, и
никаких паролей нигде не лежит.

Честно про границы этого совета: у автора форка это сработало, но поле
рассчитано на конкретный адрес, и **API того же портала диапазоны отклоняет** —
`0.0.0.0/0`, `/24` и `/16` все возвращают HTTP 500 `ip-validation-failure`
(проверено запросами). Поэтому бот и не может выпустить такой ключ за вас.
Принимает ли форма то, что отвергает её API, зависит от портала, и если у вас
она откажет — используйте второй способ.

**Автоматически.** Заполните `brawl_api_email` и `brawl_api_password`. При 403
бот сам зайдёт в портал, удалит свой прошлый ключ, выпустит новый под текущий
адрес и повторит запрос — примерно две секунды, один раз на смену адреса.
...
уходит, кроме сайта Supercell, и не пишется в логи, но если это не устраивает —
первый способ не требует его вовсе.
